...
#         transform = A.Compose([A.GaussianBlur(blur_limit=self.blur_limit, p=1)])
...
class RealESRGANDefense:
    ...
    def __call__(self, image):
        ...
        output, _ = self.upsampler.enhance(image * 255., outscale=self.outscale)

        # print('1111################################################################')
        # print(image)
        # cv2.imwrite('/home/24a_guh@lab.graphicon.ru/realesrgan.png', np.array(output, dtype=np.float32))
        image = np.array(output, dtype=np.float32) / 255.
        image = torch.from_numpy(image).cuda()
        # print('2222################################################################')
        # print(image)
        image = image.unsqueeze(0).permute(0, 3, 1, 2)
